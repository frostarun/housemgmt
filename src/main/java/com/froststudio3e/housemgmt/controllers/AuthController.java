...
public class AuthController {
	...
	public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
		...
		return ResponseEntity.ok(new JwtResponse(jwt, userDetails.getId(), userDetails.getUsername(), userDetails.getHouse(), roles));
	}
...
//		if (userRepository.existsByHouse(signUpRequest.getHouse())) {
//			return ResponseEntity.badRequest().body(new MessageResponse("Error: House is already in use!"));
//		}
		...
		String house = signUpRequest.getHouse();

		if (ObjectUtils.isEmpty(house)) {
			houseRepository.findByName(house).orElseThrow(() -> new RuntimeException("Error: House is not found."));
		} else {
			House givenHouse = houseRepository.findByName(house).orElseThrow(() -> new RuntimeException("Error: House is not found."));
			user.setHouse(givenHouse);
		}
